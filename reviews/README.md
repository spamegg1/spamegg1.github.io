...
Reviews of some online courses I took.
...
DISCLAIMER: These are my subjective personal opinions! Make your own judgment.
Also, I will talk a lot about my feelings! You are warned.

I keep mentioning something called OSSU.
[Here it is](https://github.com/ossu/computer-science/) in case you haven't heard of it.
...
- [Reviews](#reviews)
  - [Learning tips](#learning-tips)
    - [Metacognition, universal patterns](#metacognition-universal-patterns)
    - [Hitting the wall of meta-knowledge](#hitting-the-wall-of-meta-knowledge)
    - [Implicit knowledge and meta-knowledge deadlock](#implicit-knowledge-and-meta-knowledge-deadlock)
    - [Breaking the deadlock and climbing the wall](#breaking-the-deadlock-and-climbing-the-wall)
    - [Mental habits](#mental-habits)
    - [Motivation, Mental Resilience](#motivation-mental-resilience)
    - [Time](#time)
    - [Long term memory](#long-term-memory)
    - [Recognizing Pedagogy](#recognizing-pedagogy)
    - [Practical matters](#practical-matters)
    - [Loving the Abstract Void of Nothingness](#loving-the-abstract-void-of-nothingness)
  - [Harvard's CS50 (first half)](#harvards-cs50-first-half)
  - [CS50, second half, Final Project](#cs50-second-half-final-project)
  - [How to Code 1 \& 2](#how-to-code-1--2)
  - [Software Construction 1, 2](#software-construction-1-2)
  - [Programming Languages A, B, C](#programming-languages-a-b-c)
  ...
  - [Nand2Tetris 1,2](#nand2tetris-12)
  - [Intro to Networking](#intro-to-networking)
  - [Operating Systems: Three Easy Pieces](#operating-systems-three-easy-pieces)
  - [Intro to CS and Programming with Python](#intro-to-cs-and-programming-with-python)
  - [Algorithms (Tim Roughgarden)](#algorithms-tim-roughgarden)
  - [Databases](#databases)
  ...
  - [LAFF - On programming for Correctness](#laff---on-programming-for-correctness)
  - [Intro to Parallel Programming](#intro-to-parallel-programming)
  - [Functional Programming in Scala (5 courses)](#functional-programming-in-scala-5-courses)
  ...
  - [Effective Programming in Scala](#effective-programming-in-scala)
  - [Kotlin for Java Developers](#kotlin-for-java-developers)
  - [C for Everyone (non-review)](#c-for-everyone-non-review)
  - [Semantics of First Order Logic](#semantics-of-first-order-logic)
  - [Jetbrains Academy Functional Scala](#jetbrains-academy-functional-scala)
...
**Yet another thought (September 2023):** This course gives students the very strong impression of being needlessly slow, boring, difficult. (They don't see the point of some things, they want to rush through it, use other languages / tools, etc.) [Learning experts](https://youtu.be/oxZzoVp5jmI?t=677) mention that such slow, difficult ways of instruction are very good for *long term* retention and transfer of knowledge to other areas.
...
Since I've already taken [all of](#operating-systems-three-easy-pieces) [that](https://github.com/ossu/computer-science/issues/690), I'm not sure how well someone would do in this course if they haven't. Maybe there are shorter simpler sources to learn this stuff only to the extent required for web dev, although the overall state of web programmers out there seems to be "not quite understanding these topics, but muddling through somehow." Sad! I think Helsinki U students are probably familiar with all this.
...
I had *a lot of* trouble making the assignment work in IntelliJ due to Gradle errors. It's because the `build.gradle` files use deprecated features that do not work with Gradle 8.0. That shows you they are not maintaining their courses. You'd think that since the language, the IDE, the plugin and *the freaking course* are all made by the same people, it should work, right? Funnily, Week 2 that used to work also stopped working later! Anyway, we need to downgrade Gradle from 8.0 to 7.5.1. This can be done by manually copying `gradle/wrapper/gradle-wrapper.properties` file that specifies 7.5.1, along with its `.jar` file.

Week 4 is about OOP. Why is this ordered so backwards? Weird. Like Scala, Kotlin improves on Java's OOP, so they show you the differences. Like in Scala, you don't need to explicitly define or use getters and setters. It's done automatically. Then they introduce lazy evaluation (again like Scala). Kotlin also has singleton objects and companion objects (again like Scala). Then parametric types (like Java / Scala). Also covers operator overloading conventions (not present in Java obviously). The programming assignment is the usual `Rational` class that is done to death in every course I took. The second part of the assignment (creating a board game) has very poor instructions, no idea even where to start. Also the solution videos are missing! Gradle build issues continue.

Week 5 is about some advanced topics: inlining (pre-compiling and "baking into bytecode" lambda functions for performance), sequences (lazy collections that don't compute intermediate results, just like Spark), lambda with receiver (extension + lambda), and Kotlin type hierarchy (compared to Java types). Programming assignment is about [2048](https://play2048.co/). Hard stuff with barely any instructions, and solution video missing again. We are expected to read the tests and figure it out. Barely receives enough coverage. Others also [have the same Gradle issues](https://www.coursera.org/learn/kotlin-for-java-developers/discussions/forums/IrkMjr2rEei3hw68_fnwAg/threads/J5p41uJxEe2f_gpjj07RGQ). In the "Conclusion" video the instructor says "Congratulations for finishing Java for Kotlin developers". Funny! But goes to show you the level of quality control they have.
...
## Jetbrains Academy Functional Scala
...
Poor first impressions: file tabs use red colors for file names, which makes it look like there are errors in the file. For some reason all the files are indented using... *3 spaces?* And "reindent file with 2 spaces" does not work. In fact, after I manually edit the file to use 2 spaces, "reindent file with 2 spaces" reindents the file with... 3 spaces! Moreover, the course is still using Scala 2 code, with `object Main` containing old-school C-style `def main(args: Array[String])`, all of which are wrapped with lots of curly braces. And no worksheets... Come on Jetbrains, clean up your Scala 3 game already... It improves later with using colon `:` and Python-style indentation instead of curly braces. Inline comments used for assignment instructions are so long, they run off the screen and look very messy.
...
There are links to "additional materials" with links to... *blog posts* (come on...) and videos on Youtube explaining the subject, which can be viewed directly inside the IDE! But if you click on the "Youtube" button, it opens a Chrome-like browser window that runs... ADS! Like WTF?! Then, why aren't the videos included by default to make it a *proper course*? This is just... so weird. They made videos to go with the material but hid them behind "additional materials" that open in a quasi-browser window with ads. Jetbrains has an extremely strange way of doing things. I really don't understand. I don't think these are deliberate, they are just awkward nerds probably.

After some super baby stuff, things ramp up quite fast. The difficulty curve is way off... Ironically the IDE asks you to fill a "Computer Science learning curve" survey! We are told to "go read Scala documentation for collection methods" like `filter`, `map` etc. Then there is a big assignment with a ton of `enum` hierarchies. A `Cat` has a `Pattern` which has a `TriColor` which has a `TricolorSubtype` which can be one of `Calico` or `Tortie`. Similarly for other attributes of a `Cat`. Gotta track them through hierarchies. Strangely the `enum` / `sealed trait` hierarchies are covered *later* in the course... "What is an expression?" is covered all the way at the end of the course, although it should be in the beginning... just like the Kotlin course, everything is out of order.

They don't even make use of Scala's `???` for leaving things unimplemented for now but still compiling. So you have to fill all answers before you can check. Then exercises about the collection methods. `find`, `foreach`, `map`, `flatMap`, `foldLeft`. Difficulty again raises quite a lot here, especially with `foldLeft`. They teach partial application, but *without* currying (multiple parameter lists); instead by using `_` on some parameters. The code is unaligned, does not auto-format on save... There are more typos, like `mat` instead of `match`... Going east means... *subtracting* from an `x` value? One of the exercises says at the end: "Consider this an exercise." ***So weird!***

Exercise descriptions are very sparse and lacking, so there is a lot of guessing what they want us to do. They are either too hard / unexplained ("go use Sieve of Eratosthenes algorithm"), or too easy / copyable from the text. Some exercises are weirdly implemented. Like the typical "find max of a list" exercise. Normally you have to check for an empty list, and throw an exception. But not here... you have to use `Int.MinValue` apparently! Some assignments are simply confusing. We are told to define a partial function that handles division by zero... but we are not told what to do! When we look at the solution, it turns out we are actually supposed to *ignore* the division by zero... One exercise requires bitwise operations that are not explained at all. Curly braces everywhere again, Scala 2 syntax...

IntelliJ was using *so much RAM* (more than 4GB) that my operating system started killing some apps to free some memory (`oom-killer`?). Some of the assignment instructions have typos, so they ask you to do wrong things, which then fails the tests. It's possible to peek at the solutions to see what is wrong, which has a super cool GUI to show the diff between your solution and theirs.

Following sections are about pattern matching, immutability, expressions. Some good Scala-specific hidden knowledge like value classes, `copy`ing a class, `.view`, etc. But they are disorganized and randomly scattered. More difficulty spikes with advanced ideas like *the Berliner pattern* (never heard of it). There is more to talk about here but I'll spare you the details. It's all well and good... but again it all reeks of asking you to do stuff without really properly teaching. That's part of the "go fast, kinda-sorta guess stuff, skip over some stuff, Google stuff, look up blogs, and muddle through" style "learning" culture in the software industry I guess... they just don't have the time. What can you do...

*Spam's recommendation:* A bit pointless, and not of high quality (similar to the Kotlin course). It's OK I guess. You can skip it. If you are familiar with Scala, you would know most of this already. Maybe it's good for those coming from Java? I don't think so; since it does not cover Scala basics; they would not know what a `val` is! So, it does not do a good job of filling those gaps, but also of offering something good and new to Scala users either.
